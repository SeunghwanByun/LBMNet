...
class AddCoordsTh(nn.Module):
    ...
    def forward(self, input_tensor):
        """
        :param input_tensor: (batch, c, x_dim, y_dim)
        :return:
        """
...
class AddCoords(nn.Module):
    ...
    def forward(self, input_tensor):
        ...
        if self.with_r:
            rr = torch.sqrt(torch.pow(xx_channel.type_as(input_tensor) - 0.5, 2) + torch.pow(
                yy_channel.type_as(input_tensor) - 0.5, 2))
            ret = torch.cat([ret, rr], dim=1)
...
class CoordConv(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, with_r=False, **kwargs):
        ...
        self.conv = nn.Conv2d(in_size, out_channels, kernel_size=1, stride=1, padding=0, dilation=1, groups=1, bias=True)
